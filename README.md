...
팀원이 글을 던지기만 하면 정돈된 위키가 스스로 자라는 상태를 지향합니다.
상세 내용과 구현 단계는
[DEC-002](docs/decisions/DEC-002-llm-classification-and-change-history.md)에
있습니다.

1. **LLM 자동 분류** — 카테고리, 태그, 요약, 분류 근거를 LLM이 정합니다.
   현재의 키워드 규칙을 실제 LLM 호출로 교체하고 `llm_wiki/schema.md`를
   프롬프트 컨텍스트로 씁니다. 발행 승인은 사람이 그대로 맡습니다.
2. **자동 생성 탐색 메뉴** — 수기 `sidebars.ts`를 폐지하고 분류 결과에서
   메뉴를 만듭니다. 개요에서 세부로 좁혀 들어가며 **차근차근 추적 조사**할
   수 있도록 순서와 하위 그룹, 관련 문서·선행 문서 링크를 구성합니다.
3. **로그인 기반 기여 기록** — 누가 언제 무엇을 제출·수정·승인했는지를
   DB와 감사 로그에 남깁니다.
4. **문서 끝 변경 이력** — 모든 발행 문서 마지막에 기록자와 변경 내용을
   표로 남기고, 갱신 시 덧붙이며 **이전 판을 참조**할 수 있게 합니다.
