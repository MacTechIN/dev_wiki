...
4. **분류기에 실제 LLM 연결** — 현재 키워드 규칙을 LLM 호출로 교체하고,
   ...
   → 이 항목을 포함한 **최종 목표가 DEC-002 로 확정**되었습니다
   (`docs/decisions/DEC-002-llm-classification-and-change-history.md`).
   LLM 자동 분류, 분류 결과 기반 메뉴 자동 생성, 로그인 기반 기여 기록,
   문서 끝 변경 이력 네 가지이며 6 단계 구현 순서가 적혀 있습니다.
   **3번(재승인 중복 파일)은 그 1 단계**이므로 먼저 처리하세요.
